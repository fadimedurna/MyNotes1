...
public class MainActivity extends AppCompatActivity implements NoteFragment.OnNoteListInteractionListener {
    ...
    public boolean onPrepareOptionsMenu(Menu menu){ //visible and unvisible our buttons
        Log.e("onPrepareOptionsMenu new visible",
    ...
    public ArrayList<Note> retrieveNotes(){
        ...
        for (File file : files){
            ...
            String header = getPreferences(Context.MODE_PRIVATE).
                    getString(file.getName(),"No Header!");
    ...
    public void onNoteSelected(Note note) {
        editingNote =note;
        FragmentTransaction ft = getSupportFragmentManager().beginTransaction();
        ft.replace(R.id.container,EditNoteFragment.newInstance
                (readContent(editingNote)),"edit_note");
                ft.addToBackStack(null);
        ft.commit();
        displayingEditor = !displayingEditor;
        invalidateOptionsMenu();
